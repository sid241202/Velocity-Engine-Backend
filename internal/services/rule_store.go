...
func SaveNewRuleVersion(ctx context.Context, rule *models.VelocityRule, version int) error {
	...
	`, ruleRowID, w.Type, w.SizeMs, w.SlideMs, w.TimeType, w.TimestampField, w.TimestampFormat,
		...
		// LoadActiveRules below). Not fatal, by explicit user instruction
		// (2026-09-18) that this must never block rule create/edit — log
		// loudly so it isn't mistaken for a healthy environment, and keep
		// going.
...
func LoadActiveRules(ctx context.Context) (map[string]*models.RuleRecord, error) {
	...
	if err != nil {
		...
		// failing rule loading entirely. Not fatal, by explicit user
		// instruction (2026-09-18). Matches the persistence-side tolerance
		// in SaveNewRuleVersion above.
		metrics.MySQLOptionalTableMissing.WithLabelValues("window_configs").Set(1)
